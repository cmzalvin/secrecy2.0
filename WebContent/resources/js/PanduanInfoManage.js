	...
	buttons : [
			{
				...
				handler : function() {
					...
					if (college_instructorbasicform.getForm().isValid()) {
						...
						enableOfInstructorbasicform();
						seuser = college_instructorbasicform.getForm()
								.getValues();
						// stunum = Ext.getCmp("studentnum").getValue();
						ProblemPanDuanController.updateInstructorAllInfo(
								seuser, function(data) {
									if (data) {
			...
			'-',
			{
				...
				handler : function() {
					...
					college_instructorbasicform.getForm().loadRecord(
							emptysecrecy);
					instructorInfoWinInit("新增判断题信息").show();
				}
			},
			'-',
			{
			...
			'-',
			{
				...
				handler : function() {
					if (instructorgrid.getSelectionModel().hasSelection()) {
						Ext.MessageBox.confirm('提示', '你确定要删除选中的判断题?', function(
								button) {
							if (button == 'yes') {
								...
								ProblemPanDuanController.deleteInstructors(
										rList, function(data) {
											if (data) {
...
var instructorimportform = new Ext.form.FormPanel({
	...
	buttonAlign : 'center',
	frame : true,
	url : '../xlsToJsonAboutPanDuan.do',// fileUploadServlet
	width : 300,
	height : 200,
	fileUpload : true,
...
function exportInstructorsInfoExcel() {
	...
	f.action = '../exportPanDuanInfoExcel.do';
	f
			.submit({
				failure : function(form, action) {
