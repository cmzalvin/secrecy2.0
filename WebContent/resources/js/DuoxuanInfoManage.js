	...
	buttons : [ {
		...
		handler : function() {
			...
			if (college_instructorbasicform.getForm().isValid()) {
				...
				ProblemDuoXuanController.updateInstructorAllInfo(seuser, function(data) {
					if (data) {
			...
			'-',
			{
				...
				handler : function() {
					...
					college_instructorbasicform.getForm().loadRecord(
							emptysecrecy);
					instructorInfoWinInit("新增多选题信息").show();
				}
			},
			'-',
			{
			...
			'-',
			{
				...
				handler : function() {
					if (instructorgrid.getSelectionModel().hasSelection()) {
						Ext.MessageBox.confirm('提示', '你确定要删除选中的多选题?', function(
								button) {
							if (button == 'yes') {
								...
								ProblemDuoXuanController.deleteInstructors(rList,
										function(data) {
											if (data) {
...
var instructorimportform = new Ext.form.FormPanel({
	...
	buttonAlign : 'center',
	frame : true,
	url : '../xlsToJsonAboutDuoXuan.do',// fileUploadServlet
	width : 300,
	height : 200,
	fileUpload : true,
...
function exportInstructorsInfoExcel() {
	...
	f.action = '../exportDuoXuanInfoExcel.do';
	f
			.submit({
				failure : function(form, action) {
